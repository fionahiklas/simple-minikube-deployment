...
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i $HOME/.minikube/machines/minikube/id_rsa -N -L ${http_port}:localhost:80 -L ${https_port}:localhost:443 -p ${minikube_ssh_port} docker@localhost 
